(ns amerisourcebergen.main
  "Kotodama WASM entrypoint for the Amerisourcebergen clean-room actor (L4) — Clojure port.
  ...
  facts (`amerisourcebergen.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "amerisourcebergen")
(def tier "L4")
...
(def entity-specs
  [{:entity "Prescription" :plural "prescriptions" :id-prefix "amerisou_pre"
   ...
   {:entity "Claim" :plural "claims" :id-prefix "amerisou_cla"
   ...
   {:entity "Pharmacy" :plural "pharmacies" :id-prefix "amerisou_pha"
    :fields [:npi :name :network] :required [:npi :name]
    :coerce {} :refs {}}
   {:entity "Drug" :plural "drugs" :id-prefix "amerisou_dru"
    :fields [:ndc :name :strength :form] :required [:ndc :name]
    :coerce {} :refs {}}
   {:entity "Member" :plural "members" :id-prefix "amerisou_mem"
    :fields [:memberId :planId :groupId] :required []
    :coerce {} :refs {:memberId "Member"}}
   {:entity "PriorAuth" :plural "priorauths" :id-prefix "amerisou_pri"
...
(defn emit-facts
  "EAVT facts for one record: {\"amerisourcebergen.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "amerisourcebergen-compat" :tier tier :entities entities} 200])
...
(defn start! [] :amerisourcebergen-compat/ready)
